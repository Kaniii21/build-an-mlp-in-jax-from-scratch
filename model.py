...
def assign_class_labels(inputs, num_classes):
    ...
    c1= inputs[:,:num_classes]
    labels= jnp.argmax(c1, axis=1)
...
def init_mlp_params(key, layer_sizes, scale=0.1):
    ...
    """
    Build parameters for an MLP with linear layers defined by layer_sizes.
    For layer_sizes = [d0, d1, ..., dL], return a list of L dicts:
        [{'W': (d0,d1), 'b': (d1,)},
         {'W': (d1,d2), 'b': (d2,)},
         ...
         {'W': (d_{L-1}, dL), 'b': (dL,)}]
    ...
    # Split key into one subkey per layer
...
def mlp_forward(params, x):
    ...
    # Apply hidden layers: linear → ReLU
